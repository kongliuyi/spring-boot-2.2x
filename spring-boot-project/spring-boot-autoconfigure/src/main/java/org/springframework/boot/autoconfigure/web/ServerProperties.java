...
public class ServerProperties {
	...
	public static class Tomcat {
		...
		/**
		 * 是否 HTTP 1.1或之后版本 location headers 产生调用的重定向将被用来做相对或绝对重定向
		...
		/**
		 * 服务器接收和处理的最大连接数。
		 * 一旦达到极限，操作系统仍然可以接受基于“acceptCount”属性的连接。
		...
		/**
		 * 在随后的请求中， 最大空闲处理器数将被缓存重复使用。
		 * 当设置为 -1 时，缓存将不受限制，理论最大数等于最大连接数。
		...
		/**
		 * 以逗号分隔的这些正则列表，通过这些正则列表来匹配需要忽略 TLD 扫描的 jar 包。
		 * 特殊字符 '?' 和 '*' 在正则下可以被用来分别表示只匹配一个字符和零个或多个字符
		...
		public static class Accesslog {
			...
			/**
			 * 创建日志文件的目录。可以是 Tomcat 的绝对路径或相对路径。
